...
public class AStar {
    ...
    private static void generateNodes(WorldMap tileMap) {
        nodes.clear();
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                Tile point = tileMap.getTile(x, y);
                if (point.getType() == TileType.HILL) {
                    nodes.put(point, new Node(point, 2));
                } else {
                    nodes.put(point, new Node(point, 1));
                }
            }
        }
    }

    public static ArrayList<Tile> calculateAStarNoTerrain(Tile p1, Tile p2, WorldMap tileMap) {
    ...
    public static boolean isReachable(Tile start, Tile goal, WorldMap map) {
    ...
    public static int getDistance(Tile start, Tile goal, WorldMap map) {
